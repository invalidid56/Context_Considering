...
def main():
    ...
    logger.info('Tmpdir Created at '+EX_HOME+'/tmp_data')
    ...
    for i, cor in enumerate(COPORA):
        ...
        with open(os.path.join(EX_HOME, 'tmp_data', 'process_'+str(i))+'.bin', 'wb') as f:
            for gen in gens:
                for line in gen:
                    pickle.dump(line, f)
            logger.info('Temp File Created: '+'process_'+str(i)+'.bin')
